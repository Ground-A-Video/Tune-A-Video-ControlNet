...
class SparseCausalAttention(CrossAttention):
    def forward(
        ...
        key = torch.cat([key[:, [0] * clip_length], key[:, prev_frame_index]], dim=2) ## old
        key = rearrange(key, "b f d c -> (b f) d c", f=clip_length)

        value = rearrange(value, "(b f) d c -> b f d c", f=clip_length)
        value = torch.cat([value[:, [0] * clip_length], value[:, prev_frame_index]], dim=2) ## old
        value = rearrange(value, "b f d c -> (b f) d c", f=clip_length)
...
class SpatioTemporalTransformerBlock(nn.Module):
    ...
    # Fixed Error: Add 'valid' (second bool argument)
